...
class m_product extends connect {
	...
	public function get_brandId($id) {
		$sql = "SELECT * FROM product_brand WHERE product_brand_id=$id";
	...
	public function get_productPrice($price) {
	...
	public function update_cpn($brand_id,$name) {
		$sql = "UPDATE product_brand set product_brand_name = '$name' WHERE product_brand_id='$brand_id'";
		$this->exec($sql);
		return $this;	
	}
	...
	public function add_productbrand($name){
		$sql = "INSERT INTO product_brand(product_brand_name)VALUES ('$name')";
		$this->exec($sql);
		return $this;
	}
